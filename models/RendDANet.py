...
class DANetHead(nn.Module):
    ...
    def forward(self, x):
        ...
        return sasc_output, feat_sum


class PointHead(nn.Module):
    def __init__(self, in_c=513, num_classes=1, k=3, beta=0.75):
        super(PointHead, self).__init__()
        self.mlp = nn.Sequential(
            nn.Conv1d(in_channels=in_c, out_channels=256, kernel_size=1, stride=1, padding=0, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv1d(in_channels=256, out_channels=256, kernel_size=1, stride=1, padding=0, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv1d(in_channels=256, out_channels=256, kernel_size=1, stride=1, padding=0, bias=True),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x, feature, mask):
        ...
        num_points = 2048
        points = sampling_points_v2(torch.sigmoid(mask), num_points, self.k, self.beta)
    ...
    def inference(self, x, feature, mask):
        ...
        while mask.shape[-1] != x.shape[-1]:
            ...
            points_idx, points = sampling_points_v2(torch.sigmoid(mask), num_points, training=self.training)
...
class RendDANet(BaseNet):
    ...
    def __init__(self, nclass, backbone, norm_layer=nn.BatchNorm2d):
        ...
        self.rend_head = PointHead(in_c=513, num_classes=nclass)

    def forward(self, x):
        c0, c1, c2, c3, c4 = self.base_forward(x)

        mask, feature = self.da_head(c4)
...
if __name__ == "__main__":
    net = RendDANet(backbone='resnet101', nclass=1)
