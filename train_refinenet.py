...
from models import AUNet, AUNet_R16, DANet, R2UNet, SEUNet, UNetPP, UNet, RendDANet, rf101
from data_loader import get_dataloader
from util import dice_coeff, get_accuracy, get_specificity, get_sensitivity, get_precision, get_F1
from loss import DiceLoss, MixLoss, RefineLoss
...
def train_val(config):
    ...
    if config.model_type not in ['UNet', 'R2UNet', 'AUNet', 'R2AUNet', 'SEUNet', 'SEUNet++', 'UNet++',
                                 'DAUNet', 'DANet', 'AUNetR', 'RendDANet', "RendUNet", 'RefineNet']:
    ...
    for epoch in range(config.num_epochs):
        ...
        with tqdm(total=config.num_val, desc="Epoch %d / %d validation round" % (epoch + 1, config.num_epochs),
            ...
            for image, mask in val_loader:
                ...
                epoch_acc += get_accuracy(pred=pred, true=mask) * image.shape[0]
                epoch_sen += get_sensitivity(pred=pred, true=mask) * image.shape[0]
                epoch_spe += get_specificity(pred=pred, true=mask) * image.shape[0]
                epoch_pre += get_precision(pred=pred, true=mask) * image.shape[0]
                if locker == 200:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_type', type=str, default='RefineNet', help='UNet/R2UNet/AUNet/R2AUNet/SEUNet/SEUNet++/UNet++/DAUNet/DANet/AUNetR/RendDANet')
    parser.add_argument('--data_type', type=str, default='multi', help='single/multi')
    parser.add_argument('--loss', type=str, default='rend', help='bce/dice/mix/rend')
