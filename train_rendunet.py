...
def train_val(config):
    ...
    if config.model_type not in ['UNet', 'R2UNet', 'AUNet', 'R2AUNet', 'SEUNet', 'SEUNet++', 'UNet++',
                                 'DAUNet', 'DANet', 'AUNetR', 'RendDANet', "RendUNet"]:
    ...
    if config.optimizer == "sgd":
        optimizer = SGD(model.seg.parameters(), lr=1e-2, weight_decay=1e-6, momentum=0.9)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_type', type=str, default='RendUNet', help='UNet/R2UNet/AUNet/R2AUNet/SEUNet/SEUNet++/UNet++/DAUNet/DANet/AUNetR/RendDANet')
